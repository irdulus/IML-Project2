...
class PCA:
    ...
    def __plot_explained_variance(self, tuples_eig):
        ...
        x_list = list(range(1, len(tuples_eig) + 1))
        plt.bar(x_list[:15], var_explained[:15], alpha=0.85, align='center', label='Explained variance')
        plt.step(x_list[:15], cumulative_var_exp[:15], where='mid', label='Cumulative Explained variance')
        plt.axvline(n_com_90_ex_var, color='r')
        plt.axhline(90, color='r')
        plt.xticks(x_list[:15])
    ...
    def __plot_eigenvalues(self, tuples_eig):
        ...
        plt.bar(principal_components[:15], eigenvalues_plot[:15], alpha=0.85, align = 'center')
        plt.axhline(y = 1, color = 'r')
        plt.ylabel('Eigenvalues')
        plt.xlabel('Principal components')
        plt.xticks(principal_components[:15])
